...
def _extract_features(features) -> dict:
    row = features.iloc[0]
    return {
...
def analyze_acoustics_file(wav_path: str) -> dict:
    """
    Run openSMILE directly on an existing WAV file path.
    Use this when you already have a 16kHz mono WAV on disk — avoids
    the redundant ffmpeg re-encode that analyze_acoustics() would do.
    Returns a flat dict of metrics, or {"error": "..."} on failure.
    """
    try:
        features = _smile.process_file(wav_path)
        return _extract_features(features)
